...
- (void)updateGoToMenu:(NSMenu*)aMenu
{
	...
	else
	{
		...
		for(FFMatch* match in _documentSearch.allDocumentsWithMatches)
		{
			...
			NSMenuItem* item = [aMenu addItemWithTitle:[NSString stringWithCxxString:doc->path() == NULL_STR ? doc->display_name() : path::relative_to(doc->path(), to_s(self.searchFolder))] action:@selector(takeSelectedPathFrom:) keyEquivalent:key++ < '9' ? [NSString stringWithFormat:@"%c", key] : @""];
...
// ================================
// = Collapse/Expand Find Results =
// ================================

- (IBAction)takeLevelToFoldFrom:(id)sender
{
	self.windowController.showResultsCollapsed = !self.windowController.showResultsCollapsed;
}
